...
setopacity(Client *c, double opacity)
{
	...
	if (opacity >= 0.0 && opacity <= 1.0) {
		unsigned long real_opacity = (unsigned long)(0xffffffff * opacity);
...
incOp(const Arg *arg)
{
	if (selmon->sel) {
		double val = arg->f;
		selmon->sel->opacity += val;
		if (selmon->sel->opacity > 1.0)
			selmon->sel->opacity = 1.0;
		setopacity(selmon->sel, selmon->sel->opacity);
	}
}
...
manage(Window w, XWindowAttributes *wa)
{
  ...
  setopacity(c, 1.0);  // fully opaque by default
	focus(NULL);
}
